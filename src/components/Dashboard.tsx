...
const RecentTranslations = ({ entries, loading }: { entries: HistoryEntry[]; loading: boolean }) => (
...
const DownloadedPacks = () => {
    ...
    // Group by model name (first path segment after domain)
    const modelMap: Record<string, CachedFile[]> = {};
...
const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        getHistory(username)
            .then((data: HistoryEntry[]) => setHistory(Array.isArray(data) ? data.reverse() : []))
            .catch(() => setHistory([]))
